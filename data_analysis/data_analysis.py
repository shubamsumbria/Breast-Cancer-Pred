...
plt.figure(figsize=(20, 8))
df.drop('diagnosis', axis=1).corrwith(df.diagnosis).plot(kind='bar', grid=True, title="Correlation of each Feature with Diagnonis", color="lightgreen");
...
for i in df_mean, df_se, df_worst:
    sns.pairplot(data=i, hue='diagnosis', palette='crest', corner=True)
...
mean_corr_matrix = df_mean.corr()  # Correlation Matrix of mean Dataframe
