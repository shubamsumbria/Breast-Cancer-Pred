...
rg_lr.plot(kind = 'bar', color=["red","salmon"])
...
sns.heatmap(cm_dtc,annot=True,  cmap="Oranges")
...
rg_dtc.plot(kind = 'bar', color=["orangered","Lightsalmon"])
...
sns.heatmap(cm_rfc,annot=True,  cmap="YlOrRd")
...
rg_rfc.plot(kind = 'bar', color=["gold","yellow"])
...
rg_knn.plot(kind = 'bar', color=["green","lawngreen"])
...
sns.heatmap(cm_svc_l,annot=True,  cmap="Blues")
...
rg_svc_l.plot(kind = 'bar', color=["steelblue","deepskyblue"])
...
sns.heatmap(cm_svc_r,annot=True,  cmap="PuBu")
...
rg_svc_r.plot(kind = 'bar', color=["royalblue","dodgerblue"])
...
sns.heatmap(cm_gnb,annot=True, cmap="Purples")
...
rg_gnb.plot(kind = 'bar', color=["blueviolet","mediumslateblue"])
...
prfrm_df[1:].plot(kind = "bar",width=0.9, color=["tomato","coral","gold","lightgreen","deepskyblue","cornflowerblue","mediumslateblue"])
...
prfrm_df[:1].plot(kind = "bar",width=0.8,color=["tomato","coral","gold","lightgreen","deepskyblue","cornflowerblue","mediumslateblue"])
plt.title("Time Comparison of all Classifiers", fontsize=12)
plt.xlabel(("0: Time"), fontsize=12)
plt.legend(prfrm_df.columns, ncol=4 ,loc="lower center", bbox_to_anchor=(0.5, -0.3))
plt.yticks(np.arange(0, 0.20, step=0.02))
plt.show()